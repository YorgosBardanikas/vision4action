...
Function adapted from the original function in 
https://github.com/brainets/frites/tree/master/frites/conn"""
...
def conn_fit_(data, y, roi=None, times=None, mi_type='cc', gcrn=True,
             max_delay=.3, avg_delay=False, net=False, sfreq=None, 
             minorm=False, best_links=None, nLoop=None, verbose=None, 
             **kw_links):
    ...
    # indices for the sources and targets
    i_s, i_t = cfg['x_s'], cfg['x_t']
    roi_p, n_pairs = cfg['roi_p'], len(i_s)

    # keep only inter-area links
    ind = []
    for i,r in enumerate(roi_p):
        if r.count('V1') == 2 or r.count('V2') == 2 or r.count('DP') == 2 \
                        or r.count('7A') == 2 or r.count('M1') == 2: continue
        else: ind.append(i)
    i_s, i_t, roi_p = i_s[ind], i_t[ind], roi_p[ind]
    n_pairs = len(i_s)

    # keep only the best links
    if isinstance(best_links, np.ndarray):
        ind = [i for i,r in enumerate(roi_p) if r in best_links]
        i_s, i_t, roi_p = i_s[ind], i_t[ind], roi_p[ind]
        n_pairs = len(i_s)
    ...
    for n_r in range(n_roi):
        mi_xy[n_r, :] = _conn_mi(x[n_r, :, :], y, mi_type, minorm=minorm)
    mi_xy_s = mi_xy[i_s, :]
    mi_xy_t = mi_xy[i_t, :]
    ...
    for n_d in range(n_delays):
        ...
        for n_l in range(n_pairs):
            ...
            # # use a shuffled partition of the trials of the source
            # if isinstance(part, np.ndarray): _sp = _sp[...,part]
    ...
    attrs['unit'] = 'Bits'
    fit = xr.DataArray(fit, name='FIT', dims=dims, coords=coords, 
