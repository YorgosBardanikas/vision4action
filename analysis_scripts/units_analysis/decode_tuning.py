...
# 22,23,24
unit_id = 10
trial_vector = pmd_rates_common[0].isel(units=unit_id).sel(times=slice(300,500)).mean('times')
...
for tg,pmd_rates_tg in enumerate(pmd_rates_common):
    ...
    plt.plot(times[t1:t2], r[t1:t2], color=clrs[tg], lw=3)
...
plt.savefig(f'{utils.PATH}/Figures_revisions/tuning_decoding_unit_{unit_name}.svg')
# plt.show()